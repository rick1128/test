...
@ultroid_cmd(pattern="تثبيت", fullsudo=True)
...
async def unload(event):
    ...
    if zym in lsd:
        try:
            un_plug(shortname)
            await event.eor(f"**تفريغ** `{shortname}` **تم التفريغ بنجاح ♥️🧸.**", time=3)
        ...
        await event.eor(f"**لم يتم تسمية مكون إضافي ♥️🧸** `{shortname}`", time=3)


@ultroid_cmd(
    pattern=r"الغاء التثبيت( (.*)|$)",
)
async def uninstall(event):
    ...
    if zym in lsd:
        try:
            un_plug(shortname)
            await event.eor(f"**الغاء التثبيت** `{shortname}` **تم الغاء التثبيت بنجاح 🧸♥️.**", time=3)
        ...
        return await event.eor(f"**لا يوجد مكون إضافي مسمى ❤️🧸** `{shortname}`", time=3)
...
@ultroid_cmd(pattern="الحصول على إضافات( (.*)|$)", fullsudo=True)
async def get_the_addons_lol(event):
    ...
    with open(fil, "w", encoding="utf-8") as uult:
        uult.write(plug)
    await xx.edit("معباه. الآن تحميل البرنامج المساعد..")
