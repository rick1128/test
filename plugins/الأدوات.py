...
• `{i}circle`
    قم بالرد على أغنية صوتية أو متحركة للحصول على ملاحظة فيديو.
...
• `{i}hl <a link> <text-optional>`
    يدمج الرابط بمسافة بيضاء كرسالة.
...
@ultroid_cmd(pattern="ترجمه( (.*)|$)", manager=True)
async def _(event):
...
@ultroid_cmd(pattern="البوتات( (.*)|$)", groups_only=True, manager=True)
async def _(ult):
    mentions = "• **البوتات في هذه المحادثه ♥️🧸**: \n"
...
@ultroid_cmd(
    pattern="ملاحظة الفيديو$",
)
async def _(e):
    reply = await e.get_reply_message()
    if not (reply and reply.media):
        return await e.eor("`الرد على متحركه أو بصمه 🧸♥️.`")
    if "audio" in mediainfo(reply.media):
        msg = await e.eor("`جاري التحميل ♥️🧸...`")
        try:
        ...
        await e.eor("`الرد على متحركه أو ملف صوتي فقط.`")
