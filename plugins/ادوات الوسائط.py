...
@ultroid_cmd(pattern="معلومات الوسائط( (.*)|$)")
async def mi(e):
...
@ultroid_cmd(pattern="استدارة( (.*)|$)")
async def rotate_(ult):
    match = ult.pattern_match.group(1).strip()
    if not ult.is_reply:
        return await ult.eor("`رد على ميديا...`")
    if match:
        try:
            match = int(match)
        except ValueError:
            match = None
    if not match:
        return await ult.eor("`يرجى تقديم زاوية صالحة لتدوير الوسائط..`")
