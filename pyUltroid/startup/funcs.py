...
async def autopilot():
    ...
    if not channel:
        ...
        try:
            r = await ultroid_bot(
                CreateChannelRequest(
                    title="مجموعة اشعارات ريبثون",
                    about="لا تقم بحذف هذه المجموعة لأنها تحتوي على معلوماتك تنصيبك\n\n انضم @Repthon",
                    megagroup=True,
                ),
            )
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://graph.org/file/1673a7c16517e19acd4d6.jpg", "channelphoto.jpg"
        )
...
async def customize():
    ...
    try:
        ...
        await ultroid_bot.send_message(
            "botfather", f"✨ مرحبا ✨!!  انا البوت المساعد الخاص ب{sir}"
        )
        await asyncio.sleep(2)
        ...
        await ultroid_bot.send_message(
            "botfather",
            f"✨ Powerful RepthonAssistant Bot ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ @Repthon ✨",
        )
        await asyncio.sleep(2)
